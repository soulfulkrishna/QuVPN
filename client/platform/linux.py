...
Provides Linux platform support for the VPN system.
"""
import os
import sys
import fcntl
...
def setup_routing(tun_interface: str, server_ip: str, vpn_subnet: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        subprocess.check_call(["ip", "addr", "add", f"{client_ip}/{network.prefixlen}", "dev", tun_interface])
        ...
        if not default_route:
            ...
            return False, "No default route found"
        ...
        for i, part in enumerate(parts):
            if part == "via" and i + 1 < len(parts):
                ...
                default_interface = parts[i + 1]
                
        if not default_gateway or not default_interface:
            logger.warning("Failed to parse default gateway or interface")
            return False, "Failed to parse default gateway or interface"
            
        # Make sure server traffic doesn't go through the VPN
        subprocess.check_call(["ip", "route", "add", server_ip, "via", default_gateway, "dev", default_interface])
        
        # Add route for VPN subnet through the TUN interface
        subprocess.check_call(["ip", "route", "add", vpn_subnet, "dev", tun_interface])
...
def install_service(service_name: str, executable_path: str, description: str = None) -> Tuple[bool, str]:
    """
    Install the VPN client as a systemd service on Linux
    ...
    """
    if not description:
        description = "Custom Post-Quantum VPN Client"
